...
module.exports = {
    ...
    execute(message, args){
        ...
        if(args[0] == 'gif') {
            ...
            let imgChoose = memeImgFiles[Math.floor(Math.random() * memeImgFiles.length)]
            message.channel.send("You did not specify your preferred meme, so I defaulted to send an image. For further reference, the arguments for my meme command are `gif`, `img`, and `vid`.", {files: [`./media/M_img/${imgChoose}`]})
        }
    }
}